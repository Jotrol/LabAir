...
const RECT graphRect = { 200, 10, 600, 420 };
const POINT offset = { graphRect.left + 45, graphRect.bottom - 40 };
const SIZE innerSize = { graphRect.right - graphRect.left - 65, graphRect.bottom - graphRect.top - 65 };
...
void ScalePoints(POINTFLOAT points[], INT pointsLen) {
	FLOAT maxV = points[pointsLen - 1].x - points[0].x;
	FLOAT maxK = points[pointsLen - 1].y - points[0].y;

	MAX_V = points[pointsLen - 1].x;
	MAX_K = points[pointsLen - 1].y;
